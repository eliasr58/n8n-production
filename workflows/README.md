...
| `hevy-notion.json` | ja | Header | 7 | Webhook liefert nur eine `workoutId`; der Datensatz kommt per Folge-Call auf `api.hevyapp.com` mit `retryOnFail`. Dedup gegen Notion vor dem Anlegen. |
| `apple-health-notion-log.json` | ja | Header | 6 | Upsert gegen die Notion-API: erst nach dem Datum suchen, dann `PATCH` oder `POST`. Ein zweiter Lauf am selben Tag erzeugt keine Dublette. |
...
deaktiviert. Gleiches gilt für `apple-health-notion-log.json`.
