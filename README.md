...
| Hevy → Notion | Webhook liefert nur eine ID, der Datensatz kommt per Folge-Call mit `retryOnFail`, Dedup vor dem Anlegen | [`workflows/hevy-notion.json`](workflows/hevy-notion.json) |
| Apple Health → Notion | Täglicher Upsert (suchen → anlegen *oder* aktualisieren), damit ein zweiter Lauf keine Dubletten erzeugt | [`workflows/apple-health-notion-log.json`](workflows/apple-health-notion-log.json) |
...
- **Zwei Webhooks nahmen Anfragen ohne Authentifizierung entgegen** — Tagesliste
  und Apple-Health-Sync. Bei der Tagesliste ließ sich über den Request-Body
